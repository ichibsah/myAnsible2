...
$(document).ready(function () {
  ...
  $.widget("custom.catcomplete", $.ui.autocomplete, {
    _renderMenu: function (ul, items) {
      ...
      $.each(items, function (index, item) {
        ...
        if (current_id < max_items_per_cat) that._renderItemData(ul, item);
  ...
  $("#autocomplete")
    .catcomplete(
      { minLength: min_char, delay: 700 },
      {
        // select function is used to open url for selected item
        select: function (event, ui) {
          top.frames["main"].location.href = ui.item.url;
        },
        ...
        source: function (request, response) {
          ...
          if (
            window.icingaAC.shorten === undefined ||
            shorten != window.icingaAC.shorten ||
            window.icingaAC.results[window.icingaAC.shorten].expire <
              new Date().getTime()
          ) {
            $.ajax({
              async: false,
                ...
                if (data.status.hosts) {
                  $.each(data.status.hosts, function (index, item) {
                    output.push({
                      category: "Hosts",
                      url:
                        "/icinga/cgi-bin/status.cgi?search_string=" +
                        encodeURIComponent(item.host_name),
                      label: item.host_display_name,
                      local_match: new Array(
                        item.host_name,
                ...
                if (data.status.services && display_services) {
                  $.each(data.status.services, function (index, item) {
                    output.push({
                      ...
                      url:
                        "/icinga/cgi-bin/extinfo.cgi?type=2&host=" +
                        encodeURIComponent(item.host_name) +
                        "&service=" +
                        encodeURIComponent(item.service_description),
                ...
                if (data.status.hostgroups) {
                  $.each(data.status.hostgroups, function (index, item) {
                    output.push({
                      category: "Hostgroups",
                      url:
                        "/icinga/cgi-bin/status.cgi?style=overview&hostgroup=" +
                        encodeURIComponent(item.hostgroup_name),
                      local_match: new Array(
                        item.hostgroup_name,
                        item.hostgroup_alias,
                      ),
                ...
                if (data.status.servicegroups) {
                  $.each(data.status.servicegroups, function (index, item) {
                    output.push({
                      ...
                      url:
                        "/icinga/cgi-bin/status.cgi?style=overview&servicegroup=" +
                        encodeURIComponent(item.servicegroup_name),
                      local_match: new Array(
                        item.servicegroup_name,
                        item.servicegroup_alias,
                      ),
          ...
          response(
            $.grep(
              window.icingaAC.results[window.icingaAC.shorten].data,
              function (item) {
