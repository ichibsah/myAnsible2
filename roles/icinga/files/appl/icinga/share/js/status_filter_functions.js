...
function icinga_set_status_types_text(filter) {
  ...
  if (status_types == all_status_types) return_status_types_text[0] = "All";
...
function icinga_set_properties_text(filter) {
  ...
  status_properties =
    filter == "host_properties" ? host_properties : service_properties;

  if (status_properties == 0) return_properties_text[0] = "Any";
  else {
...
function icinga_set_status_types_checkboxes(filter) {
  ...
  for (var i = 0, len = valid_status_types.length; i < len; i++) {
    if (
      valid_status_types[i] != undefined &&
      status_types & i &&
      document.getElementById(filter + "_" + i) != null
    )
...
function icinga_set_status_properies(filter) {
  ...
  if (status_properties & SCHEDULED_DOWNTIME)
    document.getElementById("filter_" + t + "p_scheduled_downtime_1").checked =
      true;
  else if (status_properties & NO_SCHEDULED_DOWNTIME)
    document.getElementById("filter_" + t + "p_scheduled_downtime_2").checked =
      true;
  else
    document.getElementById("filter_" + t + "p_scheduled_downtime_3").checked =
      true;
  ...
  if (status_properties & PASSIVE_CHECKS_ENABLED)
    document.getElementById("filter_" + t + "p_passive_checks_1").checked =
      true;
  else if (status_properties & PASSIVE_CHECKS_DISABLED)
    document.getElementById("filter_" + t + "p_passive_checks_2").checked =
      true;
  else
    document.getElementById("filter_" + t + "p_passive_checks_3").checked =
      true;
  ...
  if (filter == "host_properties") {
    if (status_properties & FLAP_DETECTION_ENABLED)
      document.getElementById("filter_" + t + "p_flap_detection_1").checked =
        true;
    else if (status_properties & FLAP_DETECTION_DISABLED)
      document.getElementById("filter_" + t + "p_flap_detection_2").checked =
        true;
    else
      document.getElementById("filter_" + t + "p_flap_detection_3").checked =
        true;
  } else {
    if (status_properties & FLAP_DETECTION_DISABLED)
      document.getElementById("filter_" + t + "p_flap_detection_1").checked =
  ...
  else document.getElementById("filter_" + t + "p_state_type_3").checked = true;

  if (status_properties & MODIFIED_ATTRIBUTES)
    document.getElementById("filter_" + t + "p_modified_attributes_1").checked =
      true;
  else if (status_properties & NO_MODIFIED_ATTRIBUTES)
    document.getElementById("filter_" + t + "p_modified_attributes_2").checked =
      true;
  else
    document.getElementById("filter_" + t + "p_modified_attributes_3").checked =
      true;
  ...
  if (status_properties & NOT_ALL_CHECKS_DISABLED)
    document.getElementById(
      "filter_" + t + "p_not_all_checks_disabled_1",
    ).checked = true;
  else
    document.getElementById(
      "filter_" + t + "p_not_all_checks_disabled_2",
    ).checked = true;
...
function icinga_update_status_types(filter) {
  ...
  valid_status_types =
    filter == "host_status_types"
      ? valid_host_status_types
      : valid_service_status_types;
  ...
  if (filter == "host_status_types")
    host_status_types =
      return_status_types != 0 ? return_status_types : all_host_status_types;
  else
    service_status_types =
      return_status_types != 0 ? return_status_types : all_service_status_types;
...
function icinga_update_properties(filter) {
  ...
  status_properties += parseInt(
    $('input[name="filter_' + t + 'p_scheduled_downtime"]:checked').val(),
  );
  status_properties += parseInt(
    $('input[name="filter_' + t + 'p_acknowledge"]:checked').val(),
  );
  status_properties += parseInt(
    $('input[name="filter_' + t + 'p_active_checks"]:checked').val(),
  );
  status_properties += parseInt(
    $('input[name="filter_' + t + 'p_passive_checks"]:checked').val(),
  );
  status_properties += parseInt(
    $('input[name="filter_' + t + 'p_eventhandler"]:checked').val(),
  );
  status_properties += parseInt(
    $('input[name="filter_' + t + 'p_flap_detection"]:checked').val(),
  );
  status_properties += parseInt(
    $('input[name="filter_' + t + 'p_is_flapping"]:checked').val(),
  );
  status_properties += parseInt(
    $('input[name="filter_' + t + 'p_notifications"]:checked').val(),
  );
  status_properties += parseInt(
    $('input[name="filter_' + t + 'p_state_type"]:checked').val(),
  );
  status_properties += parseInt(
    $('input[name="filter_' + t + 'p_modified_attributes"]:checked').val(),
  );
  status_properties += parseInt(
    $('input[name="filter_' + t + 'p_state_handled"]:checked').val(),
  );
  status_properties += parseInt(
    $('input[name="filter_' + t + 'p_not_all_checks_disabled"]:checked').val(),
  );

  if (filter == "host_properties") host_properties = status_properties;
  else service_properties = status_properties;
...
function icinga_load_filters(filter) {
  ...
  $(document).ready(function () {
    ...
    $("#submit_button").css(
      "z-index",
      $("#display_filters_box").css("z-index"),
    );
...
function icinga_changed_filter_coloring(section) {
  ...
  if (set_color == true) {
    ...
    $("#" + section + "_text_cell")
      .stop(true, true)
      .delay(300)
      .animate({ backgroundColor: fade_color_to }, 2000);
  } else {
...
function icinga_filter_toggle(section) {
  ...
  if (section == "display_filters") icinga_load_filters();
  ...
  if (in_toggle == false) {
    ...
    for (var i = 0, len = filter_ids.length; i < len; i++) {
      if (
        filter_ids[i] != undefined &&
        filter_ids[i] != section &&
        document.getElementById(filter_ids[i] + "_box").style.display != "none"
      ) {
  ...
  if (section != "display_filters") icinga_changed_filter_coloring(section);
...
function icinga_apply_new_filters() {
  ...
  else url = icinga_update_url_option(url, "hostprops", host_properties);
