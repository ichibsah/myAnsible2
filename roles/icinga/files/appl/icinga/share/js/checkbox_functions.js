...
function isValidForSubmit(form) {
  ...
  if (checkboxvalue == "false") return false;
...
function enableDisableButton(form) {
  ...
  if (
    form == "tableformhostcomment" ||
    form == "tableformservicecomment" ||
    form == "tableformhostdowntime" ||
    form == "tableformservicedowntime"
  ) {
  ...
  if (form == "tableformhost" || form == "tableformservice") {
    if (
      ...
      document.forms[form].buttonCheckboxChecked.value == "true"
    ) {
      disabled = false;
    }
  }
...
function showValue(form, arg, schedule_host_check, schedule_host_svc_check) {
  if (arg != "nothing") {
    ...
    else document.forms[form].hiddenforcefield.value = "no";
