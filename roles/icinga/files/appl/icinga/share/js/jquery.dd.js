...
(function ($) {
  ...
  var dd = function (element, options) {
    ...
    var attributes = {
      actions:
        "focus,blur,change,click,dblclick,mousedown,mouseup,mouseover,mousemove,mouseout,keypress,keydown,keyup",
    ...
    ddList =
      $(sElement).prop("size") > 1 || $(sElement).prop("multiple") == true
        ? true
        : false;
    if (ddList) {
    ...
    var createA = function (currentOptOption, current, currentopt, tp) {
      ...
      var aidoptfix =
        tp == "opt" ? getPostID("postOPTAID") : getPostID("postAID");
      var aid =
        tp == "opt"
          ? aidoptfix + "_" + current + "_" + currentopt
          : aidoptfix + "_" + current;
      var arrow = "";
      var t = "";
      var clsName = "";
      ...
      if (options.useSprite != false) {
        ...
        if (options.jsonTitle == true && isJson == true) {
          ...
          arrow =
            arrow.length == 0
              ? ""
              : '<img src="' + arrow + '" align="absmiddle" /> ';
        }
      }
      var sText = $(currentOptOption).text();
      var sValue = $(currentOptOption).val();
      var sEnabledClass =
        $(currentOptOption).prop("disabled") == true ? "disabled" : "enabled";
      a_array[aid] = {
        html: arrow + sText,
        value: sValue,
        text: sText,
      ...
      aTag += ' id="' + aid + '">';
      aTag +=
        arrow + '<span class="' + styles.ddTitleText + '">' + sText + "</span>";
      if (pH !== "") {
        aTag += pH;
      }
      aTag += "</a>";
    ...
    var applyEventsOnA = function () {
      ...
      $("#" + childid + " a.enabled").bind("click", function (event) {
        ...
        if (!ddList) {
          ...
          var sText =
            options.showIcon == false ? $(this).text() : $(this).html();
          setTitleText(sText);
          //$this.data("dd").close();
          $this.close();
        }
    ...
    var createDropDown = function () {
      ...
      if (allOptions.length > options.visibleRows) {
        var margin =
          parseInt($("#" + childid + " a:first").css("padding-bottom")) +
          parseInt($("#" + childid + " a:first").css("padding-top"));
    ...
    var manageSelection = function (obj) {
      var childid = getPostID("postChildID");
      if ($("#" + childid + " a." + styles.selected).length == 1) {
      ...
      if (selectedA != undefined) {
        var oldIndex =
          actionSettings.oldIndex == undefined ||
          actionSettings.oldIndex == null
            ? a_array[selectedA].index
      ...
      if (ddList) {
        ...
        if ($("#" + elementid).prop("multiple") == true) {
          if (keyCode == 17) {
            ...
            for (
              var i = Math.min(oldIndex, currentIndex);
    ...
    var setOutOfVision = function () {
      ...
      $("#" + elementid).after(
        "<div class='" +
          styles.ddOutOfVision +
          "' style='height:0px;overflow:hidden;position:absolute;' id='" +
          sId +
          "'></div>",
      );
    ...
    var setTitleImageSprite = function (i) {
      if (options.useSprite != false) {
        var titletextid = getPostID("postTitleTextID");
        var index =
          typeof i == "undefined" ? getElement(elementid).selectedIndex : i;
        ...
        if (sClassName.length > 0) {
          ...
          if (backgroundPosition == undefined) {
            backgroundPosition =
              $("#" + id).css("background-position-x") +
              " " +
              $("#" + id).css("background-position-y");
          }
          var paddingLeft = $("#" + id).css("padding-left");
          if (backgroundImg != undefined) {
            $("#" + titletextid)
              .find("." + styles.ddTitleText)
              .attr("style", "background:" + backgroundImg);
          }
          if (backgroundPosition != undefined) {
            $("#" + titletextid)
              .find("." + styles.ddTitleText)
    ...
    var checkMethodAndApply = function () {
      ...
      if (has_handler("change") == true) {
        ...
        var currentSelectedValue =
          a_array[$("#" + childid + " a.selected").prop("id")].text;
        if (
          $.trim(oldSelectedValue) !== $.trim(currentSelectedValue) &&
          oldSelectedValue !== ""
        ) {
    ...
    var setOriginalProperties = function () {
      ...
      for (var i in getElement(elementid)) {
        if (
          typeof getElement(elementid)[i] != "function" &&
          getElement(elementid)[i] !== undefined &&
          getElement(elementid)[i] !== null
        ) {
    ...
    this.open = function () {
      if (
        $this.get("disabled", true) == true ||
        $this.get("options", true).length == 0
      )
        return;
      var childid = getPostID("postChildID");
      ...
      if (isLocal != true) {
        switch (prop) {
          ...
          case "multiple":
            getElement(elementid)[prop] = val;
            ddList =
              $(sElement).prop("size") > 0 ||
              $(sElement).prop("multiple") == true
                ? true
                : false;
            if (ddList) {
              //do something
    ...
    this.add = function (opt, index) {
      ...
      var sValue =
        objOpt.value == undefined || objOpt.value == null
          ? sText
          : objOpt.value;
      var img =
        objOpt["title"] == undefined || objOpt["title"] == null
          ? ""
          : objOpt["title"];
      var i =
        index == undefined || index == null
          ? getElement(elementid).options.length
          : index;
    ...
    //return form element
    this.form = function () {
      return getElement(elementid).form == undefined
        ? null
        : getElement(elementid).form;
    };
    this.item = function () {
