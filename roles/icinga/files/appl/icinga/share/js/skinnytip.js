...
function parseCommands(commands) {
  if (commands != null) {
    ...
    for (var i = 0; i < comArray.length; i++) {
      ...
      eval(
        "v_" + trimWhitespace(args[0]) + '="' + trimWhitespace(args[1]) + '"',
      );
    }
  }
}
...
function makeHTML(text, title) {
  ...
  txt +=
    '<div id="skinnytip_content" style="' +
    content_style +
    '">' +
    text +
    "</div>";
...
function createBackLayer(width, Z) {
  ...
  return (
    '<iframe id="backdrop" frameborder="0" scrolling="no" width="' +
    width +
    '" height="0" style="z-index: ' +
    Z +
    '; filter: Beta(Style=0,Opacity=0);"><p></iframe>'
  );
}
